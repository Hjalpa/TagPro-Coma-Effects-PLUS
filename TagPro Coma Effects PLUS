...
const settings = tpul.settings.addSettings({
    ...
    fields: {
        ...
        browncoatShine: {
            label: 'Browncoats Ball Shine',
            type: 'checkbox',
            default: true
        },
        browncoatShadow: {
            label: 'Browncoats Drop Shadow',
            type: 'checkbox',
            default: true
        }
    }
...
// Make explosions look more agressive
...
// stolen from ballcoats mod script
const initBrowncoat = () => {
    let tr = tagpro.renderer;
    let shineTexture = PIXI.Texture.fromImage("https://i.ibb.co/W3gwLKg/Donut-Shine.png");
    let dropShadow = PIXI.Texture.fromImage("https://i.ibb.co/tsrc0wT/Donut-Shadow.png");
    let defaultUpdatePlayerSpritePosition = tr.updatePlayerSpritePosition;
    tr.updatePlayerSpritePosition = function (player) {
        // Create shine
        if (settings.get("browncoatShine") && !player.sprites.shine) {
            player.sprites.shine = new PIXI.Sprite(shineTexture);
            player.sprites.ball.addChild(player.sprites.shine);
        }
        // Create drop shadow
        if (settings.get("browncoatShadow")) {
...
tagpro.ready(function() {
    ...
    if (settings.get("browncoatShine") || settings.get("browncoatShadow")) {
        initBrowncoat();
    }
});
